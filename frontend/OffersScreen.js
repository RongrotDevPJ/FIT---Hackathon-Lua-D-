...
import { 
  ...
  Text, 
  View, 
  FlatList, // <--- เปลี่ยนจาก ScrollView เป็น FlatList
  TouchableOpacity,
  Alert 
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons'; // ต้องติดตั้ง package นี้ใน project

// --- [1. Mock Data (ข้อมูลจำลอง)] ---
const offersData = [
  {
    id: 'o1',
    productName: 'ลำไยพันธุ์อีดอ',
    grade: 'AA',
    seller: 'โรงงานไทยรุ่งเรือง',
    offeredPrice: 45.50, // บาท/กก.
    offeredWeight: 5000, // กก.
    status: 'pending', // pending, accepted, rejected
    date: '10/11/2568',
  },
  {
    id: 'o2',
    productName: 'ลำไยพันธุ์สีชมพู',
    grade: 'A',
    seller: 'กลุ่มวิสาหกิจชุมชน',
    offeredPrice: 35.00,
    offeredWeight: 2500,
    status: 'accepted', 
    date: '08/11/2568',
  ...
  {
    id: 'o3',
    productName: 'ลำไยพันธุ์เบี้ยวเขียว',
    grade: 'B',
    seller: 'โรงงานส่งออกจีน',
    offeredPrice: 28.00,
    offeredWeight: 8000,
    status: 'pending', 
    date: '05/11/2568',
...
const OfferItem = ({ item }) => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case 'pending':
        return { color: '#FFB800', text: 'รอการตอบรับ' };
      case 'accepted':
        return { color: '#1E9E4F', text: 'ดีลสำเร็จ' };
      case 'rejected':
        return { color: '#D9534F', text: 'ยกเลิก/ปฏิเสธ' };
      default:
        return { color: '#888', text: 'ไม่ทราบสถานะ' };
    }
  };
  ...
  const handleViewDeal = () => {
    Alert.alert(
      'รายละเอียดดีล', 
      `คุณต้องการดูรายละเอียดข้อเสนอจาก ${item.seller} หรือไม่?\n\n(ในแอปฯ จริงจะไปที่หน้าแชท/เจรจา)`,
    );
  };
...
export default function OffersScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {/* --- [3. ใช้ FlatList แทนการเช็ค offers.length] --- */}
      <FlatList
        data={offers}
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubText}>
              เมื่อมีผู้ซื้อยื่นข้อเสนอรับซื้อประกาศของคุณ ข้อเสนอจะปรากฏที่นี่
            </Text>
          </View>
        )}
      />
...
// (โค้ด Stylesheet ทั้งหมด: ถูกแก้ไขและเพิ่ม)
const styles = StyleSheet.create({
  ...
  offerCard: {
    ...
    elevation: 3,
    borderLeftWidth: 5,
    borderLeftColor: '#1E9E4F', // สีเขียวหลัก
  },
  cardHeader: {
    flexDirection: 'row',
